...
def preference_list_to_matrix(preference_list):
    n = len(preference_list.keys())
    m = len(preference_list[0])
    matrix = np.zeros((n,m))
    for i in range(n):
        for j in range(m):
            matrix[i, preference_list[i][j]] = (m - j)/m
    return matrix

def proposals_to_matrix(proposals_dict, n, m):
    matrix = np.zeros((n,m))
...
def matching_to_matrix(proposals_dict, n, m):
    matrix = np.zeros((n,m))
    for k,w in proposals_dict.items():
        matrix[k, w] = 1
    return matrix
...
class MarriageModel:
   ...
   #############################################
    ...
    # to-do: current condition seems to be wrong. We will hardcode a rule using the fact that the number of women is the same as the number of men
    # and terminate the algorithm when all men are matched. This will fail if there are more women than men
    @staticmethod
    def __rejection_exists(PrefLists, mu, recent_proposals):
        ...
        if set(PrefLists) == set(mu):
            # print(f'PrefLists: {PrefLists}')
            # print(f'mu: {mu}')
            return False
        
        # there is no rejection if every proposer is at the end of their list
        elif sum(0 if recent_proposals[p] == PrefLists[p][-1] else 1 for p in set(PrefLists).difference(set(mu))) == 0:
...
#############################################
    ...
    def update_dataset(self, dataset, w_preferences, m_preferences, current_matching, current_proposal_matrix, next_matching_matrix, next_proposal_matrix):

        new_data = [current_matching, current_proposal_matrix, next_matching_matrix]
        keys = ['current_matching', 'current_proposal_matrix', 'next_matching_matrix']

        # append values that we do not have to deepcopy, as they are static
        # we deepcopy for now! Check later if this is necessary
        for key, val in zip([w_preferences, m_preferences], ['w_preferences', 'm_preferences']):
            dataset[val].append(deepcopy(key))
            # dataset[val].append(key)

        for key, val in zip(keys, new_data):
            dataset[key].append(deepcopy(val))
        dataset['new_proposals_matrix'].append(deepcopy(next_proposal_matrix - current_proposal_matrix))
        # print(f'current_proposal_matrix: {current_proposal_matrix}')
        # print(f'new_proposals_matrix: {deepcopy(next_proposal_matrix - current_proposal_matrix)}')
        # print(f'next_proposal_matrix: {next_proposal_matrix}')
        # print()
        # we add a 1 to the not_finished list to indicate that the algorithm is not finished
        # we will later pad with zeros to indicate that the algorithm is finished for the remaining rounds
        dataset['not_finished'].append(1)
  ...
  #############################################
    ...
    def Deferred_Acceptance(self, **kwargs):
        ...
        dataset = {
            key: [] for key in 
            ['w_preferences', 'm_preferences', 'current_matching', 'current_proposal_matrix', 'next_matching_matrix', 'new_proposals_matrix', 'not_finished']
            }
        
        m_preferences = preference_list_to_matrix(P_m)
        w_preferences = preference_list_to_matrix(P_w)
        # dataset['m_preferences'] = [m_preferences]
        # dataset['w_preferences'] = [w_preferences]

        current_matching = np.zeros((m,w))
        current_proposal_matrix = np.zeros((m,w))

        all_proposals = {p: [] for p in P_m.keys()}
        ...
        for p,Pref_list in P_m.items():
            ...
            mu.update({p: pointed})
            all_proposals[p].append(pointed)
        ...
        while rejections_exist:
            ...
            if rejections_exist and kwargs['print_tentative_matchings']:
            # if rejections_exist and kwargs.get('print_tentative_matchings') is True:
                # print('Tentative matching after Round {}:'.format(itr))
                # print(mu)
                # print(f'Matching matrix: {matching_to_matrix(mu, m, w)}')
                # print()
                # print(f'All proposals: {all_proposals}')
                # print(f'Proposals matrix: {proposals_to_matrix(all_proposals, m, w)}')
                next_proposal_matrix = proposals_to_matrix(all_proposals, m, w)
                next_matching_matrix = matching_to_matrix(mu, m, w)

                self.update_dataset(dataset, w_preferences, m_preferences, current_matching, current_proposal_matrix, next_matching_matrix, next_proposal_matrix)

                # dataset.append((
                #     tuple(deepcopy(c) for c in [w_preferences, m_preferences, current_matching, current_proposal_matrix]), 
                #     tuple(deepcopy(c) for c in [next_matching_matrix, next_proposal_matrix])
                #     )
                #                 )
                current_matching = next_matching_matrix
                current_proposal_matrix = next_proposal_matrix
